...
export function ReportsPage(): React.JSX.Element {
  ...
  const monthlyTotal = useMemo(
    () => active.reduce((s, sv) => s + monthlyCost(sv), 0),
    [active],
  );
  const yearlyTotal = useMemo(
    () => active.reduce((s, sv) => s + yearlyCost(sv), 0),
    [active],
  );
  ...
  const exportCSV = () => {
    ...
    const rows = services.map(
      (s) =>
        `${s.name},${s.category},${s.plan},${s.price},${s.priceCurrency},${s.nextRenewal},${s.active ? "فعال" : "غیرفعال"}`,
    );
    ...
    <div>
        ...
        {[
          {
            ...
            label: "هزینه ماهانه",
            value: formatMoney(monthlyTotal, "IRT", currency),
            color: "#3B82F6",
          },
          {
            icon: <TrendingUp size={18} strokeWidth={1.8} />,
            label: "هزینه سالانه",
            value: formatMoney(yearlyTotal, "IRT", currency),
            color: "#22D3EE",
          },
          {
            icon: <Layers size={18} strokeWidth={1.8} />,
            label: "تعداد سرویس",
            value: toFaDigits(active.length) + " سرویس",
            color: "#34D399",
          },
          {
            icon: <CalendarClock size={18} strokeWidth={1.8} />,
            label: "پرداخت ۳۰ روز",
            value: formatMoney(upcoming30Cost, "IRT", currency),
            color: "#F59E0B",
          },
        ].map((k) => (
        ...
        <div className={styles.table}>
          ...
          {top.map((item, i) => (
            <div key={item.service.id} className={styles.tableRow}>
              ...
              <span className={styles.tdPrice}>
                {formatMoney(item.monthly * 12, "IRT", currency)}
              </span>
            </div>
          ))}
        </div>
      ...
      <div className={styles.panel}>
        <p className={styles.panelTitle}>
          پرداخت‌های ۳۰ روز آینده ({toFaDigits(upcoming30.length)} مورد)
        </p>
        {upcoming30.length === 0 ? (
          ...
          <div className={styles.table}>
            {upcoming30.map((s) => (
              <div key={s.id} className={styles.tableRow}>
                ...
                <span className={styles.tdCat}>
                  {toFaDigits(daysUntil(s.nextRenewal))} روز دیگر
                </span>
                <span className={styles.tdPrice}>
                  {formatMoney(s.price, s.priceCurrency, currency)}
                </span>
              </div>
            ))}
          </div>
