...
export function DashboardPage(): React.JSX.Element {
  ...
  return (
    <div>
          ...
          <div className={styles.panel}>
            ...
            <div style={{ direction: 'ltr' }}>
              <ResponsiveContainer width="100%" height={160}>
                <PieChart>
                  <Pie data={byCat} dataKey="value" nameKey="label" innerRadius={45} outerRadius={70} paddingAngle={3} stroke="none">
                    {byCat.map((s) => <Cell key={s.category} fill={s.color} />)}
                  </Pie>
                  <Tooltip contentStyle={TOOLTIP} formatter={(v: number) => [formatMoneyShort(v), '']} />
                </PieChart>
              </ResponsiveContainer>
            </div>
            <div className={styles.legend}>
              {byCat.map((s) => (
                <div key={s.category} className={styles.legendItem}>
                  ...
                  <span className={styles.legendVal}>{formatMoneyShort(s.value)}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {popup && <ServiceMiniPopup service={popup} onClose={() => setPopup(null)} />}
    </div>
  );
}
