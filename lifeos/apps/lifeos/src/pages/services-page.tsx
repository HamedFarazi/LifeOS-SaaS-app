...
const SORT_OPTIONS: { value: SortKey; label: string }[] = [
  { value: 'renewal',   label: 'نزدیک‌ترین تمدید' },
  { value: 'cost-high', label: 'بیشترین هزینه' },
  { value: 'cost-low',  label: 'کمترین هزینه' },
...
export function ServicesPage(): React.JSX.Element {
  ...
  const filtered = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      switch (sort) {
        case 'renewal':   return daysUntil(a.nextRenewal) - daysUntil(b.nextRenewal);
        case 'cost-high': return monthlyCost(b) - monthlyCost(a);
        case 'cost-low':  return monthlyCost(a) - monthlyCost(b);
        case 'alpha':     return a.name.localeCompare(b.name, 'fa');
        default:          return 0;
      }
    });
  ...
  return (
    <div>
      <PageHeader title="سرویس‌ها" subtitle={`${services.length} سرویس مدیریت‌شده`} />
